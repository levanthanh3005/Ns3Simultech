...
#ifndef EPC_SGW_APPLICATION_EXT_H
#define EPC_SGW_APPLICATION_EXT_H
 ...
 */
class EpcSgwApplicationExt : public Application
{
public:
   ...
   */
  EpcSgwApplicationExt (const Ptr<Socket> s1uSocket, Ipv4Address s5Addr,
                     const Ptr<Socket> s5uSocket, const Ptr<Socket> s5cSocket);

  /** Destructor */
  virtual ~EpcSgwApplicationExt (void);
  ...
  void AddPgwS8Roaming (Ipv4Address pgwS8Address, Ptr<Socket> sgwS8Socket);

  void AddAddrToForwards8(Ipv4Address addrNew, Ipv4Address addrOld, uint64_t m_imsi);

  void AddAddrToForwards8ForTap(Ipv4Address tapAddr);

// AddAddrToForwards8ForTap(ueNodeTap.ipTap, true)
  // virtual Ipv4Address FindForwards8ByImsi (uint64_t imsi);


private:

  bool AddrNeedToForwards8(Ipv4Address addr);
  ...
  void RecvFromS8Socket (Ptr<Socket> socket);
  ...
  void SendToS8Socket (Ptr<Packet> packet, Ipv4Address pgwAddr, uint32_t teid);
  ...
  */
  ...
  Ipv4Address m_pgwS8Address;

  Ptr<Socket> m_sgwS8Socket;
  ...
  // std::vector<Ipv4Address> s8ForwardList;
  ...
  std::map<Ipv4Address, ForwardS8> s8ForwardList;
