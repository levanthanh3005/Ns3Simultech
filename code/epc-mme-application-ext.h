...
#ifndef EPC_MME_APPLICATION_EXT_H
#define EPC_MME_APPLICATION_EXT_H
 ...
 */
class EpcMmeApplicationExt : public Application
{
  /// allow MemberEpcS1apSapMme<EpcMme> class friend access
  friend class MemberEpcS1apSapMme<EpcMmeApplicationExt>;
...
public:
  ...
  /** Constructor */
  EpcMmeApplicationExt ();

  /** Destructor */
  virtual ~EpcMmeApplicationExt ();
  ...
  void AddHss (Ipv4Address hssS6aAddress, Ptr<Socket> mmeS6aSocket);

  void AddHssRoaming (Ipv4Address hssS6dAddress, Ptr<Socket> mmeS6dSocket);
  ...
  virtual void checkImsiViaRoaming(uint16_t imsi);

  virtual void setCallbackAfterCheckingRoaming (Callback<void, Ptr<LteUeNetDevice>, uint16_t> afterCheckingRoaming, Ptr<LteUeNetDevice> ptrUeLte, uint16_t enbIndex);
  ...
  virtual std::map<uint64_t, Ptr<EpcMmeApplicationExt::UeInfo> > getUeInfoMap();
  
  virtual Ptr<EpcMmeApplicationExt::UeInfo> GetUe (uint64_t imsi);
...
private:
  ...
  void RecvFromS6aSocket (Ptr<Socket> socket);

  void RecvFromS6dSocket (Ptr<Socket> socket);
  ...
  Ipv4Address m_hssS6dAddr; ///< IPv4 address of the SGW S11 interface

  uint16_t m_gtpcUdpPort; ///< UDP port for GTP-C protocol. Fixed by the standard to port 2123

  uint16_t zone = 0;

  Callback<void, Ptr<LteUeNetDevice>, uint16_t> m_afterCheckingRoaming;         //!< data received callback

  NetDeviceContainer tmpUeLte;
  uint16_t tmpEnbIndex;
  Ptr<LteUeNetDevice> tmpPtrUeLte;
};
