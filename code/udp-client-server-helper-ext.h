 ...
 */
#ifndef UDP_CLIENT_SERVER_HELPER_EXT_H
#define UDP_CLIENT_SERVER_HELPER_EXT_H
 ...
 */
class UdpServerHelperExt
{
public:
  /**
   * Create UdpServerHelperExt which will make life easier for people trying
   ...
   */
  UdpServerHelperExt (uint16_t port);
 ...
 *
 */
class UdpClientHelperExt
{

public:
  /**
   * Create UdpClientHelperExt which will make life easier for people trying
  ...
  UdpClientHelperExt (Address ip, uint16_t port);
  /**
   *  Create UdpClientHelperExt which will make life easier for people trying
   * to set up simulations with udp-client-server. Use this variant with
  ...
  UdpClientHelperExt (Address addr);
...
*/
class UdpTraceClientHelperExt
{
public:
  /**
   * Create UdpTraceClientHelperExt which will make life easier for people trying
   * to set up simulations with udp-client-server.
   *
   */
  UdpTraceClientHelperExt ();
   ...
   */
  UdpTraceClientHelperExt (Address ip, uint16_t port, std::string filename);
  /**
   * Create UdpTraceClientHelperExt which will make life easier for people trying
   * to set up simulations with udp-client-server. Use this variant with
   * addresses that do include a port value (e.g., InetSocketAddress and
   ...
   * \param filename the file from which packet traces will be loaded
   */
  UdpTraceClientHelperExt (Address addr, std::string filename);
