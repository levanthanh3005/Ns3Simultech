...
#ifndef EPC_PGW_APPLICATION_EXT_H
#define EPC_PGW_APPLICATION_EXT_H
 ...
 */
class EpcPgwApplicationExt : public Application
{
public:
   ...
   */
  EpcPgwApplicationExt (const Ptr<VirtualNetDevice> tunDevice, Ipv4Address s5Addr,
                     const Ptr<Socket> s5uSocket, const Ptr<Socket> s5cSocket);

  /** Destructor */
  virtual ~EpcPgwApplicationExt (void);
  ...
  void SendToS8Socket (Ptr<Packet> packet, Ipv4Address sgwS5uAddress, uint32_t teid);
  ...
  virtual void AddSgwS8Roaming (Ipv4Address sgwS8Addr, Ipv4Address pgwS8Addr, Ptr<Socket> pgwS8Sk);

  void AddAddrToBackwards8(Ipv4Address addr);

private:

  bool AddrNeedToBackwards8(Ipv4Address addr);
  ...
  Ipv4Address m_sgwS8Address;
  Ipv4Address m_pgwS8Address;
  Ptr<Socket> m_pgwS8Socket;

  std::vector<Ipv4Address> s8BackwardList;
