 ...
 */
#ifndef NodeExt_EXT_H
#define NodeExt_EXT_H
 ...
 * \ingroup network
 *
 * \brief A network NodeExt.
 *
 * This class holds together:
 *   - a list of NetDevice objects which represent the network interfaces
 *     of this NodeExt which are connected to other NodeExt instances through
 *     Channel instances.
 *   - a list of Application objects which represent the userspace
 *     traffic generation applications which interact with the NodeExt
 *     through the Socket API.
 *   - a NodeExt Id: a unique per-NodeExt identifier.
 *   - a system Id: a unique Id used for parallel simulations.
 *
 * Every NodeExt created is added to the NodeExtList automatically.
 */
...
public:
   ...
   */
  NodeExt(uint32_t systemId);

  virtual ~NodeExt();

  /**
   * \returns the unique id of this NodeExt.
   * 
   * This unique id happens to be also the index of the NodeExt into
   * the NodeExtList. 
   */
  ...
  */
  ...
  /**
   * In the future, ns3 NodeExts may have clock that returned a local time
   ...
   *
   * \return The time as seen by this NodeExt
   */
  ...
  /**
   * \brief Associate a NetDevice to this NodeExt.
   *
   * \param device NetDevice to associate to this NodeExt.
   * \returns the index of the NetDevice into the NodeExt's list of
   *          NetDevice.
   */
  uint32_t AddDevice (Ptr<NetDevice> device);
  /**
   * \brief Retrieve the index-th NetDevice associated to this NodeExt.
   *
  ...
  /**
   * \brief Associate an Application to this NodeExt.
   *
   * \param application Application to associate to this NodeExt.
   * \returns the index of the Application within the NodeExt's list
   *          of Application.
   */
  uint32_t AddApplication (Ptr<Application> application);
  /**
   * \brief Retrieve the index-th Application associated to this NodeExt.
   *
  ...
  /**
   * \returns the number of Application instances associated to this NodeExt.
   */
  ...
  /**
   * A callback invoked whenever a device is added to a NodeExt.
   */
  ...
  virtual std::vector<Ptr<Application> > GetApplications();

  virtual void RemoveApplication(uint32_t index);

  virtual void RemoveApplications(void);

protected:
  /**
   * The dispose method. Subclasses must override this method
   * and must chain up to it by calling NodeExt::DoDispose at the
...
private:
  ...
  /**
   * \brief Finish NodeExt's construction by setting the correct NodeExt ID.
   */
  void Construct (void);
  ...
  typedef std::vector<struct NodeExt::ProtocolHandlerEntry> ProtocolHandlerList;
  /// Typedef for NetDevice addition listeners container
  typedef std::vector<DeviceAdditionListener> DeviceAdditionListenerList;

  uint32_t    m_id;         //!< NodeExt id for this NodeExt
  uint32_t    m_sid;        //!< System id for this NodeExt
  std::vector<Ptr<NetDevice> > m_devices; //!< Devices associated to this NodeExt
  std::vector<Ptr<Application> > m_applications; //!< Applications associated to this NodeExt
  ProtocolHandlerList m_handlers; //!< Protocol handlers in the NodeExt
  DeviceAdditionListenerList m_deviceAdditionListeners; //!< Device addition listeners in the NodeExt
};
