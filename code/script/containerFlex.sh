...
SIDE_A=int-$NAME
SIDE_B=ext-$NAME
PID=$(docker inspect --format '{{ .State.Pid }}' $CONTAINERNAME)
#PID=$2
BRIDGE=br-$NAME
#INDEX=$2
...
ip netns exec $PID ip link set dev $SIDE_B name eth${ETHINDEX}
ip netns exec $PID ip link set eth${ETHINDEX} address $MAC_ADDR
ip netns exec $PID ip link set eth${ETHINDEX} up
#ip netns exec $PID ip addr add 10.12.$SEGMENT3.$SEGMENT4/16 dev eth0
ip netns exec $PID ip addr add $IP dev eth${ETHINDEX}
