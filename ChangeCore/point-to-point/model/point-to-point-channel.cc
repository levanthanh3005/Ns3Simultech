  ...
  Time txTime)
{
  ...
  // try{
  //   Ptr<MobilityModel> mobi = src->GetNode ()->GetObject<MobilityModel> ();
  //   std::cout<<"src->GetNode ():"<<src->GetNode ()->GetId()<<" "<<Names::FindName(src->GetNode ())<<std::endl;
  //   Ptr<MobilityModel> mobi = src->GetNode ()->GetObject<ns3::MobilityModel> ();
  //   std::cout<<mobi->GetPosition ()<<std::endl;
  //   // Vector srcPos = src->GetObject<MobilityModel> ()->GetPosition ();
  //   // Vector desPos = m_link[wire].m_dst->GetNode ()->GetObject<MobilityModel> ()->GetPosition ();
  //   // double currDistance = CalculateDistance (srcPos, desPos);
  //   // // double propagationDelay = currDistance / (299792458);
  //   // propagationDelay = currDistance / (100);
  //   // std::cout<<"propagationDelay:"<<propagationDelay<<std::endl;
  // } catch(int error) {
  //   propagationDelay = 0;
  // }

  Simulator::ScheduleWithContext (m_link[wire].m_dst->GetNode ()->GetId (),
                                  txTime + m_delay + (Seconds (propagationDelay)), &PointToPointNetDevice::Receive,
                                  m_link[wire].m_dst, p->Copy ());

  // Call the tx anim callback on the net device
  m_txrxPointToPoint (p, src, m_link[wire].m_dst, txTime, txTime + m_delay + (Seconds (propagationDelay)));
  return true;
}
