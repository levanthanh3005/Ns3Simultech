...
{
  // std::cout<<"Ipv4L3Protocol::Receive::"<< Names::FindName(device->GetNode()) <<" "<< p <<" "<< protocol <<" "<< from <<" "<< to <<" "<< packetType<<std::endl;;
  ...
  if (ipv4Interface->IsUp ())
  ...
  else
    {
      ...
      // std::cout<< ("Dropping received packet -- interface is down") <<std::endl;
  ...
  //   {
  //     std::cout<< ("Dropping received packet -- checksum not ok") <<std::endl;
  ...
  if (!m_routingProtocol->RouteInput (packet, ipHeader, device,
    ...
    {
      // std::cout<< ("No route found for forwarding packet.  Drop.") <<std::endl;
