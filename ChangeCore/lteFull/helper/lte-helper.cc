...
LteHelper::LteHelper (void)
  : m_fadingStreamsAssigned (false),
    ...
    m_cellIdCounter {1}
{
  // std::cout<<"LteHelper::LteHelper::Contruction"<<std::endl;
...
void 
LteHelper::setInitialValue(uint16_t cellIdCounter,   uint64_t imsiCounter)
{
  m_cellIdCounter = cellIdCounter;
  m_imsiCounter = imsiCounter;
}
...
{
  // std::cout<<"LteHelper::DoInitialize"<<std::endl;

  NS_LOG_FUNCTION (this);
  
  if (initChannelModelInitialization)
  {
...
void
LteHelper::ChangeDefaulChannelModelInitialization (bool change)
{
  initChannelModelInitialization = change;
};
...
void
LteHelper::SetUplinkSpectrumChannel (Ptr<SpectrumChannel> m_channel)
{
  m_uplinkChannel = m_channel;
}

void
LteHelper::SetDownlinkSpectrumChannel (Ptr<SpectrumChannel> m_channel)
{
  m_downlinkChannel = m_channel;
}


void
LteHelper::SetDownlinkPathlossModel (Ptr<Object>  m_model)
{
  m_downlinkPathlossModel = m_model;
};

void
LteHelper::SetUplinkPathlossModel (Ptr<Object>  m_model)
{
  m_uplinkPathlossModel = m_model;
};

void
LteHelper::SetFadingModelPointer (Ptr<SpectrumPropagationLossModel>  m_model)
{
  m_fadingModel = m_model;  
};
...
{
  // std::cout<<"LteHelper::SetEpcHelper : m_useIdealRrc: "<<m_useIdealRrc<<std::endl;
...
void
LteHelper::RemoveUeNode (Ptr<Node> ueNode)
{
...
{
  // std::cout<<"LteHelper::InstallSingleEnbDevice"<<std::endl;
  NS_LOG_FUNCTION (this << n);
...
{
  // std::cout<<"LteHelper::InstallSingleUeDevice"<<std::endl;
  NS_LOG_FUNCTION (this);
  ...
  if (m_useIdealRrc)
    {
      // std::cout<<"LteHelper::InstallSingleUeDevice::LteUeRrcProtocolIdeal"<<std::endl;
    ...
    {
      // std::cout<<"LteHelper::InstallSingleUeDevice::LteUeRrcProtocolReal"<<std::endl;
...
Ptr<NetDevice>
LteHelper::InstallSingleUeDevice (Ptr<Node> n, uint64_t m_imsi)
{
  // std::cout<<"LteHelper::InstallSingleUeDevice"<<std::endl;
  NS_LOG_FUNCTION (this);
...
Ptr<LteUeNetDevice> 
LteHelper::refreshLteUeDevice (Ptr<LteUeNetDevice> dev, uint64_t m_imsi)
{
  std::map<uint8_t, Ptr<ComponentCarrierUe> > ueCcMap = dev->GetCcMap();


for (std::map<uint8_t, Ptr<ComponentCarrierUe> >::iterator it = ueCcMap.begin (); it != ueCcMap.end (); ++it)
    {
      Ptr<LteUePhy> phy = it->second->GetPhy();
      Ptr<LteSpectrumPhy> dlPhy = phy->GetDlSpectrumPhy();
      Ptr<LteSpectrumPhy> ulPhy = phy->GetUlSpectrumPhy();

      dlPhy->SetChannel (m_downlinkChannel);
      ulPhy->SetChannel (m_uplinkChannel);
    }

  m_epcHelper->AddUe (dev, m_imsi);

  // std::cout<<"refreshLteUeDevice pass5"<<std::endl;
...
LteHelper::Attach (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
  ...
  // std::cout<<"LteHelper::Attach"<<std::endl;
  Ptr<LteUeNetDevice> ueLteDevice = ueDevice->GetObject<LteUeNetDevice> ();
  ...
  if (m_epcHelper != 0)
    {
      // std::cout<<"LteHelper::Attach::m_epcHelper->ActivateEpsBearer"<<std::endl;
      // activate default EPS bearer
  ...
  if (m_epcHelper == 0)
    {
      // std::cout<<"LteHelper::Attach::ueDevice->GetObject<LteUeNetDevice> ()->SetTargetEnb"<<std::endl;
