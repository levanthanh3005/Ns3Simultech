...
void
LteUeNetDevice::CleanCcMap ()
{
  // std::cout<<"LteUeNetDevice::CleanCcMap"<<std::endl;
  std::map< uint8_t, Ptr<ComponentCarrierUe> >::iterator it;

  for (it = m_ccMap.begin (); it != m_ccMap.end (); ++it)
    {
      it->second->GetPhy ()->Dispose ();
      it->second->GetMac ()->Dispose ();
    }
  // std::cout<<"LteUeNetDevice::CleanCcMap::2"<<std::endl;
  ...
  // std::cout<<"LteUeNetDevice::CleanCcMap::3"<<std::endl;

};

void
LteUeNetDevice::resetFrameNo ()
{
  std::map< uint8_t, Ptr<ComponentCarrierUe> >::iterator it;

  for (it = m_ccMap.begin (); it != m_ccMap.end (); ++it)
    {
      // it->second->GetMac ()->resetFrameNo (); 
      it->second->GetPhy ()->reInit();
...
{
  // std::cout<<"LteUeNetDevice::UpdateConfig"<<std::endl;
  NS_LOG_FUNCTION (this);

  if (m_isConstructed)
    {
      // std::cout<<"LteUeNetDevice::UpdateConfig::: set IMSI"<<std::endl;
...
void 
LteUeNetDevice::reInitialze(void)
{
  DoInitialize();
};
...
{
  // std::cout<<"LteUeNetDevice::DoInitialize"<<std::endl;
  NS_LOG_FUNCTION (this);
...
{
  // std::cout<<"LteUeNetDevice::Send"<<std::endl;
