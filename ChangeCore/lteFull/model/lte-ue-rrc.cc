...
LteUeRrc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteUeRrc")
    ...
    .AddTraceSource ("InitialCellSelectionEndError",
                     ...
                     "ns3::LteUeRrc::CellSelectionTracedCallback")
    .AddTraceSource ("RandomAccexssSuccessful",
...
void LteUeRrc::Disconnect()
{
  // DoDisconnect();
  // std::cout<<"LteUeRrc::Disconnect"<<std::endl;
  LeaveConnectedMode();
};
...
{
  // std::cout<<"LteUeRrc::DoSetTemporaryCellRnti::"<<rnti<<std::endl;
...
{
  // std::cout<<"LteUeRrc::DoNotifyRandomAccessSuccessful::"<<m_imsi<<" "<< m_cellId<<" "<< m_rnti <<std::endl;
...
{
  // std::cout<<"LteUeRrc::DoForceCampedOnEnb::cellId::"<<cellId<<" "<<dlEarfcn<<" m_imsi::"<< m_imsi<<" m_state::"<<m_state<<" " <<std::endl;
...
LteUeRrc::DoConnect ()
{
  // std::cout<<"LteUeRrc::DoConnect:"<<m_state<<" "<<m_imsi<<std::endl;
  ...
  // std::cout<<"LteUeRrc::after DoConnect:"<<m_state<<std::endl;
}
...
{
  // std::cout<<"LteUeRrc::DoRecvSystemInformationBlockType1"<<std::endl;
  NS_LOG_FUNCTION (this);
  switch (m_state)
    {
    case IDLE_WAIT_SIB1:
...
{
  // std::cout<<"LteUeRrc::DoRecvSystemInformation"<<std::endl;
  NS_LOG_FUNCTION (this << " RNTI " << m_rnti);
...
{
  // std::cout<<"LteUeRrc::DoRecvRrcConnectionReconfiguration"<<std::endl;
  NS_LOG_FUNCTION (this << " RNTI " << m_rnti);
      ...
      if (msg.haveMobilityControlInfo)
        {
          ...
          // std::cout<<"LteUeRrc::DoRecvRrcConnectionReconfiguration:DisposeOldSrb1"<<std::endl;
...
LteUeRrc::MaxRsrpCell
LteUeRrc::getMaxRsrpCell ()
{
  ...
  LteUeRrc::MaxRsrpCell rs;
  rs.cellId = maxRsrpCellId;
  rs.rsrp = maxRsrp;
  return rs;
};
...
{
  NS_LOG_FUNCTION (this);
  // std::cout<<"LteUeRrc::SynchronizeToStrongestCell::"<<m_state<<" vs "<<IDLE_CELL_SEARCH<<std::endl;
...
{
  // std::cout<<"LteUeRrc::EvaluateCellForSelection"<<std::endl;
  NS_LOG_FUNCTION (this);
...
{
  // std::cout<<"LteUeRrc::ApplyRadioResourceConfigDedicatedSecondaryCarrier"<<std::endl;
  NS_LOG_FUNCTION (this);
...
{
  // std::cout<<"LteUeRrc::ApplyRadioResourceConfigDedicated"<<std::endl;
...
{
  // std::cout<<"LteUeRrc::StartConnection"<<std::endl;
  NS_LOG_FUNCTION (this << m_imsi);
  NS_ASSERT (m_hasReceivedMib);
...
void
LteUeRrc::LeaveCurrentEpc ()
{
  // std::cout<<"LteUeRrc::LeaveCurrentEpc"<<std::endl;
  NS_LOG_FUNCTION (this << m_imsi);
...
{
  // std::cout<<"LteUeRrc::LeaveConnectedMode"<<std::endl;
  NS_LOG_FUNCTION (this << m_imsi);
  m_leaveConnectedMode = true;
  m_radioLinkFailureDetected.Cancel ();
  m_storedMeasValues.clear ();
  ...
  // std::cout<<"LteUeRrc::LeaveConnectedMode::"<<m_state<<std::endl;

  DoStartCellSelection (m_dlEarfcn);
  // std::cout<<"LteUeRrc::LeaveConnectedMode::DoStartCellSelection::"<<m_state<<std::endl;

  //Save the cell id UE was attached to
  StorePreviousCellId (m_cellId);
  // std::cout<<"LteUeRrc::LeaveConnectedMode::StorePreviousCellId::"<<m_state<<std::endl;
  ...
  // std::cout<<"LteUeRrc::Disconnect::SwitchToState::LeaveConnectedMode::"<<m_state<<std::endl;
...
{
  // std::cout<<"LteUeRrc::SwitchToState::"<<newState<<std::endl;
  ...
  // std::cout<<"LteUeRrc::SwitchToState::"<<m_state<<std::endl;
}
