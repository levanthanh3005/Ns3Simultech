...
void
LteEnbNetDevice::RemoveUe (uint16_t rnti)
{  
  for (uint8_t i = 0; i < m_ccMap.size (); i++)
    {
       // GetPhy (i)->RemoveUe(rnti);
       GetMac (i)->GetLteEnbCmacSapProvider()->RemoveUe(rnti);
    }
  ...
  // for (auto it = m_ccMap.begin (); it != m_ccMap.end (); ++it)
  //   {
  //      it->second->GetMac ()->GetLteEnbCmacSapProvider()->RemoveUe(rnti);
  //      it->second->GetPhy ()->RemoveUe(rnti);
  //   }
