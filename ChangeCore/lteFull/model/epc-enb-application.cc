...
{
  // std::cout<<"EpcEnbApplication::DoInitialUeMessage :"<<imsi<<" "<<rnti<<std::endl;
  NS_LOG_FUNCTION (this);
...
{
  // std::cout<<"EpcEnbApplication::DoPathSwitchRequest"<<std::endl;
  NS_LOG_FUNCTION (this);
...
{
  // std::cout<<"EpcEnbApplication::RecvFromS1uSocket::"<<m_cellId<<std::endl;
  ...
  if (it == m_teidRbidMap.end ())
    {
      // std::cout<<"UE context at cell id " << m_cellId << " not found, discarding packet"<<std::endl;
...
{
  // std::cout<<"EpcEnbApplication::SendToLteSocket::"<<m_cellId<<" -> "<<rnti<<std::endl;
